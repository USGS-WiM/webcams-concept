...
var xmlSites;
                    ...
                    $.getJSON('webcam_site_info.json', function (data) {
                        ...
                        $(siteInfo).each(function (index, site) {
                            ...
                            if (latlngArr[0] != "error") {
                                testMarkers[siteID] = L.circleMarker(latlngArr, { fillColor: "#d11010", color: "#000", weight: 0, fillOpacity: 0.6, radius: 8 }).bindPopup(customPopup);
                                sitesLayer.addLayer(testMarkers[siteID])
                                count++;
                            }
                        ...
                        function findSite(siteId) {
                            ...
                            //handle sites that aren't in the NWIS sites.xml
                            if (latlng.length > 0) {
                                return latlng;
                            } else {
            ...
            /* var sitesLayer = L.markerClusterGroup({showCoverageOnHover: false, maxClusterRadius: .1, spiderfyDistanceMultiplier: 2});
